...
- Rechtsklick pausiert das Werkzeug und lässt die Bodengalerie offen. Esc schließt Werkzeug und Galerie vollständig. Ein erneuter Klick auf Boden oder Radierer aktiviert es wieder. Gemalte Bilder liegen dauerhaft unter `worlds/<Welt-ID>/shadowrun-sprawlbuilder-painted/`; Rückgängig entfernt Tiles, nicht die erzeugten Dateien.
...
**Stufe** zeigt beim Platzieren die Vorschau an; bei Reihen den höchsten berechneten Wert. Die Buttons **− / +** ändern die angezeigte Stufe um 1 (Minimum 0) und schalten dabei auf manuell. **Automatisch stapeln** aktiviert die Berechnung wieder. Nach Abschalten der Automatik lässt sich auch eine feste nichtnegative ganze Stufe eingeben. Die Wahl wird am Tile gespeichert und im Kachelmenü bearbeitet. Automatische Einzelverschiebungen berechnen die Stufe nach dem Einrasten neu; ein zusätzlicher Speicher-Hook sichert lokale GM-Einzelverschiebungen auch außerhalb des Vorschauwegs ab. Mehrfachverschiebung behält die bestehenden Stufen; andere Kacheln werden nicht mitverschoben oder umsortiert.

Logische Stapelstufe und Foundrys interne Zeichenreihenfolge (`sort`) werden getrennt geführt. Tatsächliche Höhe und Szenenebene bleiben unverändert. Es zählen sichtbare Kacheln derselben Szenenebene; eine leere Ebenenzuordnung gilt wie in Foundry auf allen Ebenen. Unterschiedliche Meterhöhen verhindern die logische Stapelberechnung nicht. Foundry berücksichtigt diese Höhen bei der Darstellung weiterhin separat. Die angezeigte Stufe wird aus tatsächlich darunter einsortierten, überlappenden Kacheln ermittelt: Eine isolierte Bodenfläche mit interner Sortiernummer 17 ist deshalb trotzdem Stufe 0. Die interne Sortiernummer wird für die automatische Bildreihenfolge separat gesetzt. Bestehende Szenen ändern sich beim Laden nicht. Ein später daruntergemalter Boden folgt ebenfalls der Überlappungsregel; für eine bewusst untere Platzierung die Automatik ausschalten und Stufe 0 wählen.
