...
export function beginResize(){
  ...
  const overlay=document.createElement('div');overlay.className='ssb-resize';document.body.append(overlay);
  const frame=document.createElement('div');frame.className='ssb-resize-frame';overlay.append(frame);
  ...
  for(const [edge,label] of [['left','Links'],['right','Rechts'],['top','Oben'],['bottom','Unten']]){
    const handle=document.createElement('button');handle.className=`ssb-edge ssb-edge-${edge}`;handle.title=`${label} ziehen`;handle.setAttribute('aria-label',handle.title);frame.append(handle);let pointer;
